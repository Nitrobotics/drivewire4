...
public class DriveWireServer 
{
	...
	{
		// Check if jSerialComm is available (replaces RXTX)
		// jSerialComm doesn't print version messages like RXTX did
		
		try
		{
			// Just try to get the comm ports list to verify jSerialComm is working
			SerialPort[] ports = SerialPort.getCommPorts();
			logger.debug("jSerialComm loaded successfully, found " + ports.length + " serial port(s)");
			return true;
		}
		catch (Exception e)
		{
			logger.error("Failed to load jSerialComm: " + e.getMessage());
			return false;
		}
	}
	...
	public static void saveServerConfig() throws ConfigurationException
	{
	 ...
	 // jSerialComm - enumerate serial ports
	...
	public static String getSerialPortStatus(String portName)
	{
		String res = "";
		
		try
		{
			// jSerialComm - get port by name
	...
	public static boolean testSerialPort_Open(String device) throws Exception
	{
		...
		try
		{
			...
			if (port == null)
			{
				throw (new Exception("Port not found: " + device));
			}
			
			if (!port.openPort())
			{
				throw (new Exception("Could not open port (may be in use)"));
			}
