...
public class DWAPISerial {
	...
	private DWCommandResponse doCommandJoin(String[] cmd) 
	{
		...
		try
		{
			...
			if (serialPort == null)
			{
				return new DWCommandResponse(false, DWDefs.RC_SERIAL_PORTINVALID, "Port not found");
			}
			
			if (!serialPort.openPort())
			{
				return new DWCommandResponse(false, DWDefs.RC_SERIAL_PORTINUSE, "Port in use or unavailable");
			}
	...
	private DWCommandResponse doCommandShow(String port) 
	{
		...
		try
		{
			...
			int flowMode = sp.getFlowControlSettings();
