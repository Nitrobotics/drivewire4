...
public class SimpleWizard extends Wizard
{
	...
	public boolean performFinish()
	{
		...
		// Instance configuration is supplied by the server (MainWin.dwconfig).
		// On some platforms/startup sequences the wizard can be opened before that
		// config arrives, so guard against null and still apply the basic printer
		// selection (OutputDir can be adjusted later in the UI).
		org.apache.commons.configuration.HierarchicalConfiguration instCfg = MainWin.getInstanceConfig();
		if (instCfg != null)
		{
			for (int i = 0; i <= instCfg.getMaxIndex("Printer"); i++)
			{
				if (printertype.equals(instCfg.getString("Printer(" + i + ")[@name]")))
				{
		...
		{
			System.out.println("[DriveWire] SimpleWizard: instance config not available yet; setting CurrentPrinter only (OutputDir unchanged)");
			cmds.add("dw config set CurrentPrinter " + printertype);
		}
